...
**Package installs:** follow [agent-package-install-cheatsheet.md](./agent-package-install-cheatsheet.md) (`agent-pip`, `npm install -g`, `bun install -g`, etc.).
...
1. If `config.yaml` / `state.db` / `.env` exist at `**/home/agent/` root** but NOT under `**/home/agent/.hermes/`**, stop and tell the operator to run first (as root):
  ```bash
   DRY_RUN=1 migrate-volume-to-home.sh /home/agent
   migrate-volume-to-home.sh /home/agent
  ```
   Then restart the service before continuing.
2. If `~/.hermes/workspace` still exists with project files, tell the operator to run `migrate-volume-to-home.sh /home/agent` (it moves/merges into `/home/agent/workspace`).
3. Record baseline:
  ```bash
